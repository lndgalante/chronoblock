...
def batch_response(from_block: int, to_block: int, missing: set[int] | None = None) -> list[dict]:
    missing = missing or set()
    return [
        {
            "jsonrpc": "2.0",
            "id": block - from_block,
            "result": None if block in missing else {
                "number": hex(block),
...
class TestFetchBlockTimestamps:
    ...
    async def test_stops_at_missing_block_in_successful_batch(self):
        ...
        chain = Chain(id=1, name="ethereum", rpc="http://example.test", rpc_batch_size=4, rpc_concurrency=1, finality_blocks=64)
    ...
    async def test_empty_prefix_when_all_batches_fail(self):
        ...
        chain = Chain(id=1, name="ethereum", rpc="http://example.test", rpc_batch_size=2, rpc_concurrency=1, finality_blocks=64)
